...
@objcMembers public class PropertyListHandler: NSObject {
    
    /// 将工程文件内容转为字典对象
    ///
    /// - Parameter fileURL: 文件路径 URL
    /// - Returns: 字典对象，即转化后的内容
    public static func parseProject(fileURL: URL) -> [String: Any]? {
        var url = fileURL
    ...
    ///   - fileURL: 工程文件路径
    ///   - list: 数据对象
    public static func generateProject(fileURL: URL, withPropertyList list: Any) {
        var url = fileURL
    ...
    /// - Returns:  是否还原成功
    public static func recoverProject(fileURL: URL) -> Bool {
        var url = fileURL
        let backupURL = backupURLOf(projectURL: &url)
        do {
            if FileManager().fileExists(atPath: backupURL.path) {
    ...
    public static func apply(json: [String: Any], onProjectData projectData: [String: Any], forward isForward: Bool = true) -> [String: Any] {
    ...
    public static func compare(project project1: [String: Any], withOtherProject project2: [String: Any]) -> Any {
    ...
    ///   - modified: 修改过的工程文件
    ///   - original: 原始工程文件
    public static func generateJSON(filePath: String, withModifiedProject modified: URL, originalProject original: URL) {
    ...
    /// - Returns: 数据对象
    public static func parseJSON(fileURL url: URL) -> Any? {
