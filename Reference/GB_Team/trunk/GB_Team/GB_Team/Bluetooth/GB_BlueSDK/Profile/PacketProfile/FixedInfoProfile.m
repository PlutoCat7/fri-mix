...
@implementation FixedInfoProfile

#pragma mark - Protected 子类必须实现的方式
- (GattPacket *)createGattPacket {
    
    return [GattPacket createFixInfoPacketGatt];
}

- (BOOL)isCurrentProfileResponse:(GattPacket *)packet {
    return packet.head == PACKET_HEAD && packet.type == GATT_READ_FIX_INFO_PACK_BACK;
}
...
- (void)doGattPacketProfile:(PacketProfile*)profile packet:(GattPacket *)packet {
    ...
    NSArray *result = [GattPacket parseFixInfoPacketGatt:packet.data];
    if (!result) {
