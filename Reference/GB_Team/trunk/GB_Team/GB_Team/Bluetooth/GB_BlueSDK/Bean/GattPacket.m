...
-(id)initWithData:(NSData *)charachteristicData error:(NSError **)error {
    if (self = [super init]) {
        if (charachteristicData == nil || [charachteristicData length] < 3 || [self checkCharachteristicData:charachteristicData] == NO) {
            *error = [[NSError alloc] initWithDomain:@"gatt packet invalid from GattPacket initWithData." code:GattErrors_InvalidArgument userInfo:nil];
            return self;
        }
        ...
        if (headByte == UPDATE_BACK_HEAD || headByte == SEARCHSTAR_HEAD || headByte == Invalid_Commond) {
            self.head = headByte;
...
-(BOOL) checkCharachteristicData:(NSData *)charachteristicData {
    ...
    UInt8 headByte;
    [charachteristicData getBytes:&headByte length:1];
    if (headByte == UPDATE_BACK_HEAD && [charachteristicData length] == 4) {
        return YES;
            
    } else if (headByte == SEARCHSTAR_HEAD && [charachteristicData length] == 4) {
        return YES;
            
    } else if (headByte == Invalid_Commond && [charachteristicData length] == 4) {
        return YES;
...
+(GattPacket *)createFirewareVerPacketGatt {
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.head = PACKET_HEAD;
    gattPacket.type = GATT_FIREWARE_VER_PACKET;
        
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_FIREWARE_VER_PACKET;
    dataByte[3] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:3] length:3];
...
+(GattPacket *)createFirewarePacketGatt:(UInt16)index data:(NSData *)data {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.head = UPDATE_WRITE_HEAD;
        
    NSMutableData *gattData = [NSMutableData new];
        
    Byte dataByte[3] = {0};
    dataByte[0] = UPDATE_WRITE_HEAD;
    dataByte[1] = lowByte;
...
+(GattPacket *)createAGPSPacketGatt:(UInt8)count orderIndex:(UInt8)orderIndex packetIndex:(UInt8)packetIndex data:(NSData *)data {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.head = SEARCHSTAR_HEAD;
    
    NSMutableData *gattData = [NSMutableData new];
    
    Byte dataByte[4] = {0};
    dataByte[0] = SEARCHSTAR_HEAD;
...
+(GattPacket *)createFixInfoPacketGatt {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_READ_FIX_INFO_PACK;
    
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_READ_FIX_INFO_PACK;
    dataByte[3] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:3] length:3];
...
+(GattPacket *)createVariableInfoPacketGatt {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_READ_VARIABLE_INFO_PACK;
    
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_READ_VARIABLE_INFO_PACK;
    dataByte[3] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:3] length:3];
...
+(NSDate *)parseDeviceDateGatt:(NSData *)packetData; {
    if (packetData && packetData.length == 10) {
        ...
        comps.second = secondByte;
        comps.minute = minuteByte;
        comps.hour = hourByte;
        [comps setDay:dayByte];
        [comps setMonth:monthByte];
        [comps setYear:2000+yearByte];
        
        return [[[NSCalendar alloc]
                         initWithCalendarIdentifier:NSGregorianCalendar] dateFromComponents:comps];
    }
    return nil;
}
...
+ (NSArray<NSString *> *)parseFixInfoPacketGatt:(NSData *)packetData {
    ...
    if (packetData && packetData.length == 15) {
        ...
        [list addObject:[NSString stringWithFormat:@"%04d-%02d-%02d", yearByte+2000, monthByte, dayByte]];

        UInt8 oneByte = bytes[6];
        UInt8 pointByte = bytes[7];
        UInt8 pOneByte = bytes[8];
        UInt8 pTwoByte = bytes[9];
        
        [list addObject:[NSString stringWithFormat:@"%c%c%c%c", oneByte, pointByte, pOneByte, pTwoByte]];
...
+ (NSArray<NSString *> *)parseVariableInfoPacketGatt:(NSData *)packetData {
    ...
    if (packetData && packetData.length == 11) {
        ...
        //0：定位失败 1：定位成功 2：GPS未开启
        UInt8 locate = bytes[5];
        [list addObject:@(locate)];
        
        UInt32 startCount = bytes[6] + (bytes[7] << 8) + (bytes[8] << 16) + (bytes[9] << 24);
        [list addObject:@(startCount)];
