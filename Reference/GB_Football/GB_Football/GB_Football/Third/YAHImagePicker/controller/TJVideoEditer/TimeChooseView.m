...
@property (nonatomic, assign) CGFloat oneSecondWidth; //每秒宽度
...
-(void)setupUI{
    ...
    _startTime = (KtotalTimeForSelf-self.videoLimitTime)/2;
    _endTime = KtotalTimeForSelf-(KtotalTimeForSelf-self.videoLimitTime)/2;
    ...
    self.oneSecondWidth = imageShowW;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSInteger i = 0; i<count; i++) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [view drawImage:image inRect:CGRectMake(i*imageShowW, 0, imageShowW, self.bounds.size.height)];
    ...
    //添加裁剪范围框  居中显示
    CGFloat padding = (KimageCount-self.videoLimitTime)/2*imageShowW;
    self.startView = [[UIImageView alloc]initWithFrame:CGRectMake(padding, 0, KendTimeButtonWidth, self.bounds.size.height)];
    ...
    self.endView = [[UIImageView alloc]initWithFrame:CGRectMake(self.bounds.size.width-padding-KendTimeButtonWidth, 0, KendTimeButtonWidth, self.bounds.size.height)];
    ...
    if (self.getTimeRange) {
        self.getTimeRange(_startTime,_endTime,_startTime);
    }
}
...
-(void)panAction:(UIPanGestureRecognizer *)panGR{
    ...
    switch (view.tag) {
        case 99:
        {
            ...
            if((oldOrigin.x+P.x <= CGRectGetMaxX(self.endView.frame)-self.bounds.size.width/3.0f && oldOrigin.x+P.x>=0)){ //判断最小区域
                
                if ((oldOrigin.x+P.x >= CGRectGetMaxX(self.endView.frame)-self.oneSecondWidth*self.videoLimitTime)) {  //判断最大区域
        ...
        case 100:
        {
            ...
            if (oldOrigin.x+P.x+KendTimeButtonWidth-self.startView.frame.origin.x>=self.bounds.size.width/3.0f && oldOrigin.x+P.x+KendTimeButtonWidth<=self.bounds.size.width) {//判断最小区域
                
                if (oldOrigin.x+P.x+KendTimeButtonWidth-self.startView.frame.origin.x<=self.oneSecondWidth*self.videoLimitTime) {  //判断最大区域
                    view.frame = CGRectMake(oldOrigin.x+P.x, 0,KendTimeButtonWidth, self.bounds.size.height);
                }
            }
        }
