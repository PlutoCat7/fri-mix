...
@interface GBGuestGameCompletePreview ()
...
@implementation GBGuestGameCompletePreview
-(void)localizeUI{
    ...
    self.tipStLabel1.text = LS(@"post.guest.popbox.label.message1");
...
+ (void)showWithComplete:(void(^)(NSInteger index))complete dataList:(NSArray<NSString *> *)dataList {
    ...
    NSArray *xibArray= [[NSBundle mainBundle]loadNibNamed:@"GBGuestGameCompletePreview" owner:nil options:nil];
    GBGuestGameCompletePreview *preView = [xibArray firstObject];
