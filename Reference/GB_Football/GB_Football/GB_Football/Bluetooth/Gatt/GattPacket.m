...
-(id)initWithData:(NSData *)charachteristicData error:(NSError **)error
{
    if (self = [super init]) {
        ...
        } else if ([self checkCharachteristicData:charachteristicData] == NO) {
            // 判断头信息，如果是无效指令把指令码抛出去
            UInt8 headByte;
            [charachteristicData getBytes:&headByte length:1];
            if (headByte == Invalid_Commond) {
                *error = makeError(GattErrors_InvalidCommond, LS(@""));
                return self;
...
-(BOOL) checkCharachteristicData:(NSData *)charachteristicData
{
    ...
    if (headByte == UPDATE_HEAD && [charachteristicData length] == 4) {
        ...
        // if (headByte == Invalid_Commond) { 暂时不要用通知，存在逻辑问题
...
+(GattPacket *)createFlashCleanGatt:(FlashCleanType)cleanType {
    ...
    gattPacket.type = GATT_RUN_CLEAN_PACKET;
    
    Byte dataByte[5] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x05;
    dataByte[2] = GATT_RUN_CLEAN_PACKET;
    dataByte[3] = cleanType;
...
+(GattPacket *)createRunPacketCountGatt:(UInt8)month day:(UInt8)day {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_RUN_PACKET_COUNT;
    ...
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 6;
    dataByte[2] = GATT_RUN_PACKET_COUNT;
    dataByte[3] = month;
    dataByte[4] = day;
    dataByte[5] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:5] length:5];
...
+(GattPacket *)createRunPacketGatt:(UInt16)index {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_RUN_PACKET;
...
+(GattPacket *)createRunTimePacketGatt {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_RUN_TIME_PACKET;
    
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_RUN_TIME_PACKET;
...
+(GattPacket *)createSportStepPacketCountGatt:(UInt8)month day:(UInt8)day {
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_SPORT_STEP_PACKET_COUNT;
    ...
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 6;
    dataByte[2] = GATT_SPORT_STEP_PACKET_COUNT;
    dataByte[3] = month;
    dataByte[4] = day;
    dataByte[5] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:5] length:5];
...
+(GattPacket *)createSportStepPacketGatt:(UInt8)month day:(UInt8)day index:(UInt16)index {
    ...
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_SPORT_STEP_PACKET;
    ...
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 8;
    dataByte[2] = GATT_SPORT_STEP_PACKET;
    dataByte[3] = month;
    dataByte[4] = day;
    dataByte[5] = lowByte;
    dataByte[6] = heightByte;
...
+(GattPacket *)createDeviceMacPacketGatt {
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_DEVICE_MAC_PACKET;
    
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_DEVICE_MAC_PACKET;
    dataByte[3] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:3] length:3];
...
{
    GattPacket *gattPacket = [[GattPacket alloc] init];
    gattPacket.type = GATT_Close_PACKET;
    
    Byte dataByte[4] = {0};
    dataByte[0] = PACKET_HEAD;
    dataByte[1] = 0x04;
    dataByte[2] = GATT_Close_PACKET;
    dataByte[3] = [GattPacket calculateVerifyValue:[[NSData alloc] initWithBytes:dataByte length:3] length:3];
...
+(NSInteger)parseRunPacketCountGatt:(NSData *)packetData {
    ...
    return [self parseSportPacketCountGatt:packetData];
}
+(NSData *)parseRunPacketGatt:(NSData *)packetData {
    
    return [self parseSportPacketGatt:packetData];
}
+(NSInteger)parseRunIndex:(NSData *)packetData {
    
    return [self parseSportIndex:packetData];
}
...
+(NSDictionary *)parseRunTime:(NSData *)packetData {
    ...
    if (packetData && packetData.length >= 8) {
        ...
        UInt8 day = bytes[4];
        UInt8 hour = bytes[5];
        UInt8 minute = bytes[6];
        UInt8 second = bytes[7];
...
+ (NSArray<NSNumber *> *)parseBatteryGatt:(NSData *)packetData;
{
    ...
    if (packetData && packetData.length == 6) {
        ...
        UInt8 isCharge = bytes[4];
        [list addObject:@(battery)];
        [list addObject:@(isCharge)];
        return [list copy];
    }
    return nil;
}
...
+ (NSString *)parseDeviceMacGatt:(NSData *)packetData {
    if (packetData && packetData.length == 10) {
        Byte *bytes = (Byte *)[packetData bytes];
        UInt8 oneByte = bytes[3];
        UInt8 twoByte = bytes[4];
        UInt8 theByte = bytes[5];
        UInt8 fouByte = bytes[6];
        UInt8 fivByte = bytes[7];
        UInt8 sixByte = bytes[8];
        
        NSString *result = [NSString stringWithFormat:@"%02x:%02x:%02x:%02x:%02x:%02x", sixByte, fivByte, fouByte, theByte, twoByte, oneByte];
...
+ (FlashCleanType)parseCleanFalshGatt:(NSData *)packetData {
    ...
    if (packetData && packetData.length == 5) {
        ...
        UInt8 cleanType = bytes[3];
        
        return cleanType;
    }
    return FlashCleanType_Unknow;
}
...
+(NSDictionary *)transSportPacketGatt:(NSData *)packetData
{
    ...
    if (packetData && packetData.length > 13) {
        ...
        int32_t longitude = baseBytes[5] + (baseBytes[6] << 8) + (baseBytes[7] << 16) + (baseBytes[8] << 24);
        int32_t latitude = baseBytes[9] + (baseBytes[10] << 8) + (baseBytes[11] << 16) + (baseBytes[12] << 24);
...
+(NSDictionary *)transRunPacketGatt:(NSData *)packetData {
    ...
    return [self transSportPacketGatt:packetData];
}

+(NSDictionary *)transSportStepPacketGatt:(NSData *)packetData {
    if (packetData && packetData.length > 2) {
        Byte *baseBytes = (Byte *)[[packetData subdataWithRange:NSMakeRange(0, 2)] bytes];
        UInt8 month = baseBytes[0];
        UInt8 day = baseBytes[1];
        ...
        NSMutableArray *itemArray = [NSMutableArray new];
        while (start < packetData.length) {
            if (start + 7 > packetData.length) {
                break;
            }
            Byte *itemDayBytes = (Byte *)[[packetData subdataWithRange:NSMakeRange(start, 7)] bytes];
            UInt8 hour = itemDayBytes[0];
            UInt8 minute = itemDayBytes[1];
            UInt8 type = itemDayBytes[2];
            
            UInt16 duration = itemDayBytes[3] + (itemDayBytes[4] << 8);
            UInt16 steps = itemDayBytes[5] + (itemDayBytes[6] << 8);
            ...
            [itemDict setObject:[NSNumber numberWithUnsignedInt:hour] forKey:@"hour"];
            [itemDict setObject:[NSNumber numberWithUnsignedInt:minute] forKey:@"minute"];
            [itemDict setObject:[NSNumber numberWithUnsignedInt:type] forKey:@"type"];
            [itemDict setObject:[NSNumber numberWithUnsignedInt:duration] forKey:@"duration"];
            [itemDict setObject:[NSNumber numberWithUnsignedInt:steps] forKey:@"steps"];
