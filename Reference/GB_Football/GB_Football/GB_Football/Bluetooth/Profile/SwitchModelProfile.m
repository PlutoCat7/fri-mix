...
- (void)gattPacketProfile:(PacketProfile*)profile packet:(GattPacket *)packet {
    ...
    if (packet.type == (0x07|0x80)) {
        NSInteger code = [GattPacket parseSwitchModelGatt:packet.data];
        NSError *error = nil;
        if (code != 0) {
            error = makeError(code==1?SwitchModelCode_Fail:SwitchModelCode_Charging, LS(@"bluetooth.write.fail"));
        }
