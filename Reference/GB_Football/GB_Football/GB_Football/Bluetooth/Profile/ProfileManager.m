...
- (NSString *)getErrorUUIDs {
    ...
    NSString *serviceUUID = self.packetService.UUID.UUIDString;
    NSString *readUUID = self.notifyCharacteristic.UUID.UUIDString;
    NSString *writeUUID = self.packetCharacteristic.UUID.UUIDString;
    if (!serviceUUID) {
        serviceUUID = @"";
    }
    if (!readUUID) {
    ...
    return [NSString stringWithFormat:@"service:%@, read:%@, write:%@", serviceUUID, readUUID, writeUUID];
}
...
- (void)peripheral:(CBPeripheral *)aPeripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    if (!error) {
        if ([service.UUID isEqual:self.packetService.UUID]) {
            [self processCharacteristics];
...
- (void)peripheral:(CBPeripheral *)aPeripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    //GBLog(@"%@ didUpdateValueForCharacteristic", [self class]);
    if (!error) {
        //GBLog(@"%@", [characteristic value]);
        if ([characteristic.UUID isEqual:self.notifyCharacteristic.UUID]) {
            NSError *error;
            //返回数据为空，不处理
...
- (void)setPeripheral:(CBPeripheral *)peripheral {
    ...
    self.ibeaconVersion = iBeaconVersion_None;
