...
@property (nonatomic, weak) id <AKCollectionViewLayoutDelegate> delegate;
@end
...
- (void)initialize
{
    self.interitemSpacing = 40.0;//iphone6
    self.maskDisabled = false;
	self.font = self.font ?: [UIFont fontWithName:@"BEBAS" size:27];
	self.highlightedFont = self.highlightedFont ?: [UIFont fontWithName:@"BEBAS" size:27];
	...
	self.pickerViewStyle = self.pickerViewStyle ?: AKPickerViewStyleFlat;
...
- (CGFloat)offsetForItem:(NSUInteger)item
{
	NSAssert(item < [self.collectionView numberOfItemsInSection:0],
			 @"item out of range; '%td' passed, but the maximum is '%td'", item, [self.collectionView numberOfItemsInSection:0]);
