...
+ (NSURLSessionTask *)POST:(NSString *)urlString parameters:(id)parameters responseClass:(Class)responseClass handler:(RequestCompleteHandler)handler {
    ...
    return [[NetworkManager sharedNetworkManager] POST:urlString parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (![AFNetworkReachabilityManager sharedManager].reachable) {
            ...
            BLOCK_EXEC(handler, nil, [NSError errorWithDomain:LS(@"login.tip.fail") code:RequestErrorCodeHTTP userInfo:nil]);
            //添加错误报告
            if (!KGBDebug) {
...
+ (NSDictionary *)reportDefaultParameters {
    ...
    NSString *deviceVersion = [GBBluetoothManager sharedGBBluetoothManager].iBeaconObj.deviceVersion;
    NSString *fireVersion = [GBBluetoothManager sharedGBBluetoothManager].iBeaconObj.firewareVersion;
    if (!deviceVersion) {
        deviceVersion = @"";
    }
    if (!fireVersion) {
    ...
    NSDictionary *parameters = @{@"firmware_vname":fireVersion,
                                 @"hardware_vname":deviceVersion,
                                 @"phone":userInfo.phone?userInfo.phone:@"",
                                 @"user_id":@(userInfo.userId),
                                 @"sid":userInfo.sid?userInfo.sid:@"",
                                 @"client_vcode":[NSString stringWithFormat:@"%td", [UIDevice appVersionCode]],
