...
- (void)addBadgePoint:(NSInteger)pointRadius withPointPosition:(CGPoint)point {
    ...
    badgeView.tag = kTagBadgePointView;
    badgeView.layer.cornerRadius = pointRadius;
    badgeView.backgroundColor = [UIColor colorWithHexString:@"0xf13837"];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _monkeyView.image = [YLGIFImage imageNamed:@"logingif.gif"];
...
- (void)configWithType:(EaseBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError offsetY:(CGFloat)offsetY reloadButtonBlock:(void (^)(id))block{
    ...
    if (!_tipLabel) {
        ...
        _tipLabel.textColor = kRKBNAVBLACK;
    ...
    if (!_actionButton) {//新增按钮
        _actionButton = ({
            ...
            button.backgroundColor = kTiMainBgColor;
            button.titleLabel.font = [UIFont systemFontOfSize:18];
            [button setTitleColor:kRKBNAVBLACK forState:UIControlStateNormal];
    ...
    if (hasError) {
        ...
        switch (_curType) {
            case EaseBlankPageTypeBudet: {
                imageName = @"easeBlankPage_Budet";
                tipStr = @"装个理想的新家要多少钱？快来做预算啦";
                buttonTitle = @"开始做预算";
            }
                break;
    ...
    [_monkeyView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        //            }
        make.size.mas_equalTo(_monkeyView.image.size);
    }];
    ...
    [_tipLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        if (titleStr.length > 0) {
            ...
            make.top.equalTo(_monkeyView.mas_bottom).offset(35);
        }
    }];
