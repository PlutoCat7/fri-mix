...
@interface TallyBudgetView()<UITableViewDataSource, UITableViewDelegate>
...
@implementation TallyBudgetView
...
- (id)init {
    ...
    if (self) {
        ...
        [tableView registerClass:[TallyBudgetViewCell class] forCellReuseIdentifier:@"cell"];
        ...
        self.budgetArray = [[NSMutableArray alloc] initWithCapacity:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TallyBudgetViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    
    NSInteger timestamp = [[self.budgetArray[indexPath.row] objectForKey:@"budgetupdatetime"] integerValue] / 1000;
    NSString *name = [self.budgetArray[indexPath.row] objectForKey:@"budgetname"];
    [cell setInfo:name upDate:[NSDate timeStringFromTimestamp:timestamp formatter:@"yyyy-MM-dd HH:mm:ss"] isSelected:indexPath.row == self.selectedIndex];
    return cell;
}
...
//    }
    if (_selectedBlock) {
        _selectedBlock([Budget mj_objectWithKeyValues:self.budgetArray[indexPath.row]]);
    }
