...
typedef NS_ENUM(NSInteger , MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE) {
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DEFAULT,//默认
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ALLCOLLECTION,//美家征集 -> 查看全部
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ADVERTISEMENTS,//广告
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE,//发现用户发的文章
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY,//发现用户发的照片
    MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_SHOWALL,//显示全部
};


@interface MyNoticedDiscoveryViewController () <UITableViewDelegate,UITableViewDataSource,SwitchOptionsViewDelegate,MoreCollectionCellDelegate,SDPhotoBrowserDelegate,GBSharePanDelegate>
...
@implementation MyNoticedDiscoveryViewController
...
{
    WEAKSELF;
    [[TiHouse_NetAPIManager sharedManager] request_myConcernDiscoveryListWithPage:0 usingBlock:^(NSArray *data, NSError *error) {
...
{
    WEAKSELF;
    [[TiHouse_NetAPIManager sharedManager] request_myConcernDiscoveryListWithPage:self.discoveryDataSources.count usingBlock:^(NSArray * data, NSError *error) {
...
- (void)resetViewModelWithShouldRemoveAll:(BOOL)shouldRemoveAllFirst dataModel:(NSArray *)dataModelArray
{
    ...
    if (dataModelArray && dataModelArray.count > 0)//如果有发现图文 或者 文章
    {
        for (int i = 0; i < dataModelArray.count; i++)
        {
            ...
            MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DEFAULT;//是否是文章 还是图片类型
            ...
            {
                cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE;
            ...
            {
                cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY;
                operatedFirstImage = dataModel.assemarciszan ? [UIImage imageNamed:@"find_root_has_like"] : [UIImage imageNamed:@"find_root_like"];
                operatedFirstTitle = [NSString stringWithFormat:@"%ld",dataModel.assemarcnumzan];
            ...
            if (1)//如果有作者信息
            {
                [sectionArray addObject:[self authorViewModelWithImageUrl:dataModel.urlhead topTitle:dataModel.username bottomTitle:dataModel.createtimeStr rightButtonCanTouch:!dataModel.assemarcisconcern dataModel:dataModel cellType:cellType]];
            ...
            if (cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY)//如果是发现的照片
            {
                if (moreLineText.length > 0)//如果有文字
                {
                    [sectionArray addObject:[self moreLineLabelViewModelWithText:moreLineText textColor:RGB(0, 0, 0) lineNumber:kMaxLine cellType:MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY dataModel:dataModel]];
                    if (hasShowAllButton)
                    {
                        [sectionArray addObject:[self moreLineLabelViewModelWithText:@"全文" textColor:RGB(83, 134, 168) lineNumber:1 cellType:MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_SHOWALL dataModel:dataModel]];
                    }
                    [sectionArray addObject:[self emptyModelWithHeight:15 hasBottomLine:NO backgroundColor:[UIColor whiteColor] cellType:cellType dataModel:dataModel]];
                }
                ...
                if (dataModel.assemarcfileJA.count && dataModel.assemarcfileJA.count > 0)//如果有照片
                {
                    ...
                    else//大于等于3张
                    {
                        ...
                        for (int i = 0 ; i < count; i++)
                        {
                            ...
                            [sectionArray addObject:[self emptyModelWithHeight:2 hasBottomLine:NO backgroundColor:[UIColor whiteColor] cellType:MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DEFAULT dataModel:nil]];
                            countPlus++;
                        }
                    }
                }
            }
            else if (cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE)//如果是文章
            {
                [sectionArray addObject:[self advertisementsOptionImageViewModelWithImageUrl:dataModel.urlindex cellType:MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE dataModel:dataModel]];
            ...
            if (cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE || cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY)//如果有操作栏
            {
            ...
            [sectionArray addObject:[self emptyModelWithHeight:15 hasBottomLine:NO backgroundColor:[UIColor clearColor] cellType:MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DEFAULT dataModel:nil]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (viewModel.cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ALLCOLLECTION)//美家征集 -> 查看全部
    {
        FindAssemListViewController *vc = [[FindAssemListViewController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
    }
    else if (viewModel.cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_DISCORVERY || viewModel.cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE)//如果是文章或者照片
    {
        if (dataModel)
        {
            //文章
    ...
    else if (viewModel.cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_SHOWALL)//全文
    {
        ...
        for (BaseViewModel *tmpViewModel in sectionViewModel)
        {
            ...
            else if (tmpViewModel.cellType == MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_SHOWALL)
            {
                if (moreTextViewModel.lineNumber == 0)//如果已经是展开全部 那么代表点击是收起
                {
...
- (BaseViewModel *)leftRightLabelViewModel
{
    ...
    viewModel.cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ALLCOLLECTION;
    viewModel.cellLineViewModel.bottomLineEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 10);
    return viewModel;
}

- (BaseViewModel *)emptyModelWithHeight:(NSInteger)height hasBottomLine:(BOOL)hasBottomLine backgroundColor:(UIColor *)backgroundColor cellType:(MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE)cellType dataModel:(id)dataModel
{
...
- (BaseViewModel *)authorViewModelWithImageUrl:(NSString *)imageUrl topTitle:(NSString *)topTitle bottomTitle:(NSString *)bottomTitle rightButtonCanTouch:(BOOL)rightButtonCanTouch dataModel:(id)dataModel cellType:(NSInteger)cellType
{
...
- (BaseViewModel *)authorViewModelWithImageUrl:(NSString *)imageUrl topTitle:(NSString *)topTitle bottomTitle:(NSString *)bottomTitle dataModel:(id)dataModel
{
    ...
    viewModel.currentCellHeight = 56;
    viewModel.cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ADVERTISEMENTS;
    viewModel.cellClass = [AuthorCell class];
    viewModel.cellLineViewModel.bottomLineEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 10);
    return viewModel;
}
...
- (BaseViewModel *)moreLineLabelViewModelWithText:(NSString *)text textColor:(UIColor *)textColor lineNumber:(NSInteger)lineNumber cellType:(MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE)cellType dataModel:(FindAssemarcInfo *)dataModel
{
...
- (BaseViewModel *)threePictureViewModel:(NSString *)imageUrl secondImageUrl:(NSString *)secondImageUrl thirdImageUrl:(NSString *)thirdImageUrl dataModel:(FindAssemarcInfo *)dataModel index:(NSInteger)index
{
    ...
    viewModel.cellType = index;//代表是第几张
    return viewModel;
}

- (BaseViewModel *)operatedIconViewModelWithFirstImage:(UIImage *)fImage firstTitle:(NSString *)fTitle secondImage:(UIImage *)sImage secondTitle:(NSString *)sTitle thirdImage:(UIImage *)tImage thirdTitle:(NSString *)tTitle dataModel:(id)dataModel cellType:(MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE)cellType
{
...
- (BaseViewModel *)advertisementsOptionImageViewModelWithImageUrl:(NSString *)imageUrl cellType:(MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE)cellType dataModel:(id)dataModel
{
...
- (BaseViewModel *)morelLineLabelWithBackgroundViewModelWithText:(NSString *)text font:(float)font textColor:(UIColor *)textColor dataModel:(id)dataModel
{
    ...
    viewModel.cellType = MYNOTICEDDISCOVERYVIEWCONTROLLERELLTYPE_ARTICLE;
    viewModel.dataModel = dataModel;
    return viewModel;
}
...
{
    [self checkThreePhotoWithViewModel:viewModel andPhotoIndex:viewModel.cellType * 3];
}

- (void)threePictureCell:(ThreePictureCell *)cell clickCenterImageWithViewModel:(ThreePictureViewModel *)viewModel;
{
    [self checkThreePhotoWithViewModel:viewModel andPhotoIndex:viewModel.cellType * 3 + 1];
}

- (void)threePictureCell:(ThreePictureCell *)cell clickRightImageWithViewModel:(ThreePictureViewModel *)viewModel;
{
    [self checkThreePhotoWithViewModel:viewModel andPhotoIndex:viewModel.cellType * 3 + 2];
}
