...
@interface MineFindMainViewController() <UITableViewDelegate, UITableViewDataSource, MineFindMainContentCellDelegate, GBSharePanDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) MineFindMainHeader *tableHeader;
...
@implementation MineFindMainViewController
...
-(MineFindMainHeader *)tableHeader{
    if (!_tableHeader) {
        _tableHeader = [[MineFindMainHeader alloc]init];
...
- (void)getFindData {
    ...
    WEAKSELF
    [[TiHouseNetAPIClient sharedJsonClient] requestJsonDataWithPath:@"/api/inter/assemarc/page" withParams:@{@"uid": @(user.uid), @"start": @((_pager.page - 1) * _pager.perPage), @"limit": @(_pager.perPage)} withMethodType:Post autoShowError:YES andBlock:^(id data, NSError *error) {
        if ([data[@"is"] intValue]) {
