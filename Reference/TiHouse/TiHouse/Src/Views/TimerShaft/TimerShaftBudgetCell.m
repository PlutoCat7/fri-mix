...
#define BOTTOMHIGHT  30

#import "TimerShaftBudgetCell.h"
#import "NSDate+Common.h"
#import "BudgetCell.h"

@interface TimerShaftBudgetCell()<UITableViewDelegate, UITableViewDataSource>
...
@implementation TimerShaftBudgetCell
...
- (void)setmodelTunerBudget:(TunerBudget *)tunerBudget needTopView:(BOOL)needTopView needBottomView:(BOOL)needBottomView{
    _tunerBudget = tunerBudget;
    
    _topView.height = needTopView ? 20 : 0;
    _icon.y = CGRectGetMaxY(_topView.frame);
    
    _dateView.text = [[NSDate dateWithTimeIntervalSince1970:_tunerBudget.latesttime/1000] stringDisplay_MMdd];
    ...
    if ([_tunerBudget.listModelLogbudgetope count] > 0) {
        _budgetTitleView.text = @"预算有了新动态: ";
    } else {
        _budgetTitleView.text = @"添加了一版新预算: ";
    }
    [_budgetTitleView sizeToFit];
    
    _budgetNameView.text = tunerBudget.budgetname;
    ...
    //预算内容
    _tableView.height = tunerBudget.listModelLogbudgetope.count * 35;
    _tableView.y = CGRectGetMaxY(_budgetNameView.frame) + 15;
    
    //用户名及发布时间
    NSString *timeViewStr = [NSString stringWithFormat:@"%@，%@",_tunerBudget.nickname,[[NSDate dateWithTimeIntervalSince1970:_tunerBudget.latesttime/1000] stringDisplay_HHmm]];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return _tunerBudget.listModelLogbudgetope.count;
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    BudgetCell *cell = [tableView dequeueReusableCellWithIdentifier:@"BudgetCell"];
    cell.Budget = _tunerBudget.listModelLogbudgetope[indexPath.row];
    if (indexPath.row == _tunerBudget.listModelLogbudgetope.count -1) {
...
+ (CGFloat)cellHeightWithTunerBudget:(TunerBudget *)tunerBudget needTopView:(BOOL)needTopView needBottomView:(BOOL)needBottomView{
    ...
    cellHeight += 35*tunerBudget.listModelLogbudgetope.count;
    cellHeight += 34 + 8;
    cellHeight += 49;//按钮高度
    cellHeight += needBottomView ? BOTTOMHIGHT : 0;
...
-(UIButton *)checkBudget{
    if (!_checkBudget) {
        ...
        [_checkBudget setTitle:@"查看预算" forState:(UIControlStateNormal)];
...
-(UIButton *)checkRevamp{
    if (!_checkRevamp) {
        ...
        [_checkRevamp setTitle:@"查看修改" forState:(UIControlStateNormal)];
