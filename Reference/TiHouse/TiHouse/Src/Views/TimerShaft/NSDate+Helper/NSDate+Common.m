...
- (NSInteger)hoursAgo:(NSDate *)date{
    NSCalendar *calendar = [[self class] sharedCalendar];
    NSDateComponents *components = [calendar components:(NSHourCalendarUnit)
                                               fromDate:self
                                                 toDate:date
...
-(NSString *)stringTimeCha:(NSDate *)date{
    
    NSInteger agoCount = [self hoursAgo:date];
    if (agoCount > 24) {
        return [NSString stringWithFormat:@"%@ - %@",[self stringWithFormat:@"yyyy年MM月dd日 HH:mm"],[date stringWithFormat:@"yyyy年MM月dd日 HH:mm"]];
    }else{
        return [NSString stringWithFormat:@"%@  %@ - %@",[self stringWithFormat:@"yyyy年MM月dd日"],[self stringWithFormat:@"HH:mm"],[date stringWithFormat:@"HH:mm"]];
    }
}
...
- (NSString *)stringDisplay_HHmm{
    ...
    } else if ([self leftDayCount] == -6) {
        displayStr = @"六天前";
    } else if ([self leftDayCount] == -5) {
        displayStr = @"五天前";
    } else if ([self leftDayCount] == -4) {
        displayStr = @"四天前";
    } else if ([self leftDayCount] == -3) {
        displayStr = @"三天前";
    } else if ([self leftDayCount] == -2) {
        displayStr = @"两天前";
    } else if ([self leftDayCount] == -1) {
        displayStr = @"一天前";
...
- (NSString *)stringDisplay_MMdd{
    NSString *displayStr = @"";
    if ([self year] != [[NSDate date] year]) {
        displayStr = [self stringWithFormat:@"yy年MM月dd日"];
    } else if ([self leftDayCount] == -1) {
        displayStr = @"昨天";
    } else if ([self leftDayCount] != 0){
        displayStr = [self stringWithFormat:@"MM月dd日"];
    }else{
        displayStr = @"今天";
    }
//    }else if ([self hoursAgo] > 0){
