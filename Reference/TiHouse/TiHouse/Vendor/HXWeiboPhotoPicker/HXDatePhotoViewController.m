...
//            [self.navigationController pushViewController:vc animated:NO];
            TOCropViewController *cropController = [[TOCropViewController alloc] initWithCroppingStyle:TOCropViewCroppingStyleDefault image:model.
                                                    thumbPhoto];
            //                    cropController.aspectRatioPreset = TOCropViewControllerAspectRatioPresetSquare;
            if (self.manager.configuration.movableCropBox &&
                self.manager.configuration.movableCropBoxEditSize) {
                ...
                cropController.customAspectRatio = CGSizeMake(self.manager.configuration.movableCropBoxCustomRatio.x, self.manager.configuration.movableCropBoxCustomRatio.y);
            }else{
                cropController.aspectRatioPreset = TOCropViewControllerAspectRatioPresetSquare;
            }
            if (self.manager.configuration.singleSelected) {
            ...
            if (self.manager.configuration.ToCarpPresetSquare) {
                cropController.aspectRatioPreset = TOCropViewControllerAspectRatioPresetSquare;
            }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        if (self.manager.configuration.showDateSectionHeader) {
            HXPhotoDateModel *firstDateModel = [self.dateArray objectAtIndex:0];
            firstModel = firstDateModel.photoModelArray[0];
        }else {
            firstModel = self.allArray[0];
        }
        [cell setImageViewImage:!(firstModel.type == HXPhotoModelMediaTypeVideo)];
        // 为了最后一个icon 显示拍照还是拍摄
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        if (!self.manager.configuration.singleSelected) {
            NSInteger currentIndex = [self.photoArray indexOfObject:cell.model];
            HXDatePhotoPreviewViewController *previewVC = [[HXDatePhotoPreviewViewController alloc] init];
            previewVC.delegate = self;
            ...
            if (cell.model.subType == HXPhotoModelMediaSubTypePhoto) {
                ...
                [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:size contentMode:PHImageContentModeDefault options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                    ...
                    TOCropViewController *cropController = [[TOCropViewController alloc] initWithCroppingStyle:TOCropViewCroppingStyleDefault image:result];
//                    cropController.aspectRatioPreset = TOCropViewControllerAspectRatioPresetSquare;
                    if (self.manager.configuration.movableCropBox &&
                        self.manager.configuration.movableCropBoxEditSize) {
                        if (CGPointEqualToPoint(self.manager.configuration.movableCropBoxCustomRatio, CGPointZero)) {
                    ...
                    cropController.cropView.cropBoxResizeEnabled = !self.manager.configuration.shouldLockCrop;
                    cropController.rotateButtonsHidden = self.manager.configuration.hiddenRotationButton;
                    cropController.toolbar.resetButton.hidden = self.manager.configuration.hiddenResetButton;
                    cropController.aspectRatioLockEnabled= YES;
                    cropController.resetAspectRatioEnabled = NO;
                    cropController.delegate = self;
                    cropController.doneButtonTitle = @"完成";
...
//                [self.navigationController pushViewController:vc animated:NO];
                ...
                if ([VideoTtimmerViewController sharedInstance].model) {
                    [[VideoTtimmerViewController sharedInstance] destorySharedInstance];
                }
                VideoTtimmerViewController *videoTtimmerVC = [VideoTtimmerViewController sharedInstance];
                videoTtimmerVC.model = cell.model;
                videoTtimmerVC.delegate = self;
                [self.navigationController pushViewController:videoTtimmerVC animated:YES];
                return;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if ([kind isEqualToString:UICollectionElementKindSectionHeader] && self.manager.configuration.showDateSectionHeader) {
        ...
        headerView.allSelectBlock = ^{
            __strong typeof(weakSelf) strongSelf = weakSelf;
            __strong typeof(weakHeaderView) strongHeaderView = weakHeaderView;

            [strongSelf handleAllSelectedWithSection: indexPath.section];
            strongHeaderView.model = strongSelf.dateArray[indexPath.section];
        };
        return headerView;
    }
...
//        }
//    }else {
//        return CGSizeMake(self.view.hx_w, 0);
//    }
//}
...
// MARK: - handle all selected 全选/取消全选
- (void)handleAllSelectedWithSection:(NSUInteger)section {
    
    HXPhotoDateModel *dateModel = [self.dateArray objectAtIndex:section];
    for (int i = 0, count = (int)dateModel.photoModelArray.count; i < count; i++) {
        HXDatePhotoViewCell *cell = (HXDatePhotoViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:i inSection:section]];
        if ([cell isKindOfClass:[HXDatePhotoViewCell class]]) {
            cell.selectBtn.selected = dateModel.isAllSelected;
            [self datePhotoViewCell:cell didSelectBtn:cell.selectBtn];
        }
    }
    dateModel.isAllSelected = !dateModel.isAllSelected;
//    if ((dateModel.isAllSelected == NO && dateModel.photoModelArray.count > 20) == NO) {
//        dateModel.isAllSelected = !dateModel.isAllSelected;
//    }
}
...
{
    HXPhotoModel *model = [HXPhotoModel photoModelWithVideoURL:url videoTime:videoTime];
    model.asset = assest;
    if (self.manager.configuration.singleSelected) {
        [self.manager beforeSelectedListAddPhotoModel:model];
        [self datePhotoBottomViewDidDoneBtn];
...
- (HXDatePhotoViewFlowLayout *)customLayout {
    if (!_customLayout) {
        ...
        _customLayout.sectionInset = UIEdgeInsetsMake(2, 0, 2, 0);
        //        if (iOS9_Later) {
...
-(AddHouseTableViewCell *)touView{
    if (!_touView) {
        _touView = [[AddHouseTableViewCell alloc]initWithFrame:CGRectMake(0, kNavigationBarHeight, self.view.width, 50)];
        _touView.Title.text = @"记录时间";
        _touView.TextField.text = @"拍摄时间";
        _touView.TextField.userInteractionEnabled = NO;
        _touView.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        [self.view addSubview:_touView];
//        [_touView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setImageViewImage:(BOOL)isPhtoo
{
    self.imageView.image = [UIImage imageNamed:isPhtoo ? @"photo_cra" : @"photo_video"];
}
...
-(UIImageView *)imageView{
    if (!_imageView) {
        ...
        _imageView.image = [UIImage imageNamed:@"photo_cra"];
    }
    return _imageView;
}
...
@property (strong, nonatomic) UIImageView *saveIcon;
...
- (void)setModel:(HXPhotoModel *)model {
    ...
    self.saveIcon.hidden = ![TweetImage UserSaveImageDate:model.creationDate];
...
- (void)setSelectedTitleColor:(UIColor *)selectedTitleColor {
    ...
    [self.selectBtn setTitleColor:[UIColor clearColor] forState:UIControlStateSelected];
}
...
- (void)layoutSubviews {
    ...
    self.saveIcon.hx_x = 0;
    self.saveIcon.bottom = self.hx_h;
...
- (UIImageView *)imageView {
    if (!_imageView) {
        ...
        [_imageView addSubview:self.saveIcon];
    }
    return _imageView;
}
...
- (UIImageView *)saveIcon {
    if (!_saveIcon) {
        _saveIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"XW_saveImage"]];
        _saveIcon.hx_size = CGSizeMake(20, 20);
    }
    return _saveIcon;
}
...
//            self.dateLb.frame = CGRectMake(8, 4, self.hx_w - 16, 30);
            self.dateLb.frame = CGRectMake(8, 4, self.hx_w - 16 - 100, 30);
...
//            self.dateLb.frame = CGRectMake(8, 0, self.hx_w - 16, 50);
            self.dateLb.frame = CGRectMake(8, 0, self.hx_w - 16 - 100, 50);
...
//                    weakSelf.dateLb.frame = CGRectMake(8, 4, weakSelf.hx_w - 16, 30);
                    weakSelf.dateLb.frame = CGRectMake(8, 4, self.hx_w - 16 - 100, 30);
...
//        self.dateLb.frame = CGRectMake(8, 0, self.hx_w - 16, 50);
    ...
    self.selectBtn.frame = CGRectMake(self.hx_w - 8 - 100, 0, 100, 50);
    [self.selectBtn setTitle:model.isAllSelected ? @"取消全选" : @"全选" forState:UIControlStateNormal];
}

// MARK: - IBActions
- (void)didClickOnAllSelected {
...
//        self.subTitleLb.frame = CGRectMake(8, 26, self.hx_w - 16, 20);
        self.dateLb.frame = CGRectMake(8, 4, self.hx_w - 16 - 100, 30);
        self.subTitleLb.frame = CGRectMake(8, 26, self.hx_w - 16 - 100, 20);
    }else {
    }
...
- (UIButton *)selectBtn {
    if (!_selectBtn) {
        ...
        [_selectBtn setTitleColor:[UIColor colorWithHexString:@"0x5386a8"]  forState:UIControlStateNormal];
        [_selectBtn addTarget:self action:@selector(didClickOnAllSelected) forControlEvents:UIControlEventTouchUpInside];
    }
    return _selectBtn;
}
...
- (void)setSelectCount:(NSInteger)selectCount {
    ...
    self.doneBtn.backgroundColor = self.doneBtn.enabled ? kTiMainBgColor : [kTiMainBgColor colorWithAlphaComponent:0.5];
    [self changeDoneBtnFrame];
...
- (void)layoutSubviews {
    ...
    self.doneBtn.frame = CGRectMake(0, 0, 50, 36);
    self.doneBtn.center = CGPointMake(self.doneBtn.center.x, 25);
    [self hideBorderInView:self.bgView];
    [self changeDoneBtnFrame];
}
...
- (UIButton *)previewBtn {
    if (!_previewBtn) {
        ...
        [_previewBtn setTitleColor:kRKBNAVBLACK forState:UIControlStateNormal];
...
- (UIButton *)doneBtn {
    if (!_doneBtn) {
        ...
        [_doneBtn setTitleColor:kRKBNAVBLACK forState:UIControlStateNormal];
        [_doneBtn setTitleColor:[kRKBNAVBLACK colorWithAlphaComponent:0.5] forState:UIControlStateDisabled];
...
- (UIButton *)originalBtn {
    if (!_originalBtn) {
        ...
        [_originalBtn setTitleColor:kRKBNAVBLACK forState:UIControlStateNormal];
        [_originalBtn setTitleColor:[kRKBNAVBLACK colorWithAlphaComponent:0.5] forState:UIControlStateDisabled];
    }
    return _originalBtn;
}
- (UIButton *)editBtn {
