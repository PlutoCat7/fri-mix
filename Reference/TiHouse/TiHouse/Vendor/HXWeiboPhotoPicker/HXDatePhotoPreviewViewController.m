...
@interface HXDatePhotoPreviewViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,HXDatePhotoPreviewBottomViewDelegate,HXDatePhotoEditViewControllerDelegate,TOCropViewControllerDelegate>
...
//    [self wr_setNavBarBarTintColor:kRKBViewControllerBgColor];
//    [self wr_setNavBarTitleColor:kRKBNAVBLACK];
//    [self wr_setNavBarTintColor:kRKBNAVBLACK];
    
    self.view.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
    [self wr_setNavBarBarTintColor:[UIColor colorWithWhite:0 alpha:0.5]];
    ...
    //保护
    if (self.currentModelIndex<0 || self.currentModelIndex>=self.modelArray.count) {
...
- (void)changeSubviewFrame {
    ...
    CGFloat bottomViewHeight = self.view.hx_h - bottomHeight - bottomMargin;
    self.bottomView.frame = CGRectMake(0, bottomViewHeight, self.view.hx_w, bottomHeight + bottomMargin);
...
- (void)cropViewController:(TOCropViewController *)cropViewController didCropToImage:(UIImage *)image withRect:(CGRect)cropRect angle:(NSInteger)angle {
    ...
    HXPhotoModel *beforeModel = cropViewController.model;
    HXPhotoModel *afterModel = [HXPhotoModel photoModelWithImage:image];
...
- (HXDatePhotoPreviewBottomView *)bottomView {
    if (!_bottomView) {
        if (self.outside) {
            _bottomView = [[HXDatePhotoPreviewBottomView alloc] initWithFrame:CGRectMake(0, self.view.hx_h - 143 - kBottomMargin, self.view.hx_w, bottomHeight + kBottomMargin) modelArray:self.manager.afterSelectedArray manager:self.manager];
        }else {
            _bottomView = [[HXDatePhotoPreviewBottomView alloc] initWithFrame:CGRectMake(0, self.view.hx_h - 143 - kBottomMargin, self.view.hx_w, bottomHeight + kBottomMargin) modelArray:self.manager.selectedArray manager:self.manager];
        }
...
- (UIButton *)selectBtn {
    if (!_selectBtn) {
        ...
        [_selectBtn setBackgroundImage:[HXPhotoTools hx_imageNamed:@"Photo_-unselected@2x.png"] forState:UIControlStateNormal];
