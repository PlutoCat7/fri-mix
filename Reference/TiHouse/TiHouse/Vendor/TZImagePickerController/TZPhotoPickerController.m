    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray *models) {
...
//        }
        XWPhotoDateModel *model = _models[section];
        return model.photoModelArray.count;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TZAssetModel *model;
    XWPhotoDateModel *PhotoDateModel = _models[indexPath.section];
    if (tzImagePickerVc.sortAscendingByModificationDate || !_showTakePhotoBtn) {
        model = PhotoDateModel.photoModelArray[indexPath.row];
    } else {
        model = PhotoDateModel.photoModelArray[indexPath.row - 1];
    }
...
#pragma mark ----- 重用的问题
    ...
    if (indexPath.section == 0) {
        header.isAllSelect = NO;
        header.leftLabel.text = @"记录时间";
        header.rightLabel.text = @"拍摄时间2017-12-20";
        
    }else{
        header.isAllSelect = YES;
        header.leftLabel.text = @"2017年12月20日";
...
- (void)scrollCollectionViewToBottom {
    ...
    if (_shouldScrollToBottom && _models.count > 0) {
        XWPhotoDateModel *PhotoDateModel = _models.lastObject;
        NSInteger item = 0;
        if (tzImagePickerVc.sortAscendingByModificationDate) {
            item = PhotoDateModel.photoModelArray.count - 1;
            if (_showTakePhotoBtn) {
                TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
                if (tzImagePickerVc.allowPickingImage && tzImagePickerVc.allowTakePicture) {
                    item += 1;
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [_collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:item inSection:1] atScrollPosition:UICollectionViewScrollPositionBottom animated:NO];
...
- (void)checkSelectedModels {
    ...
    for (XWPhotoDateModel *PhotoDateModel in _models) {
        
        
        for (TZAssetModel *model in PhotoDateModel.photoModelArray) {
            model.isSelected = NO;
