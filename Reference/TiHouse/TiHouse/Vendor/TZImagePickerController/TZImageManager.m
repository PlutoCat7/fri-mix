...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    
    __block NSMutableArray *sameDayArray;//同一时间的数字
    __block NSDate *currentIndexDate;
    __block XWPhotoDateModel *dateModel;
    ...
    if ([result isKindOfClass:[PHFetchResult class]]) {
        ...
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (model) {
                NSLog(@"===========");
                NSDate *photoDate = model.creationDate;
                //        CLLocation *photoLocation = photoModel.location;
                if (!currentIndexDate) {
                    dateModel = [[XWPhotoDateModel alloc] init];
                    dateModel.date = photoDate;
                    sameDayArray = [NSMutableArray array];
                    [sameDayArray addObject:model];
                    [photoArr addObject:dateModel];
                    model.dateItem = sameDayArray.count - 1;
                    model.dateSection = photoArr.count - 1;
                }else {
...
//        NSString *creationDate = [NSString stringWithFormat:@"%@",phAsset.creationDate];
        NSDate *creationDate = phAsset.creationDate;
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        model = [TZAssetModel modelWithAsset:asset type:type timeLength:timeLength creationDate:creationDate];
    } else {
        ...
        if (type == TZAssetModelMediaTypeVideo) {
            ...
            NSString *creationDate = [NSString stringWithFormat:@"%@",[asset valueForProperty:ALAssetPropertyDate]];
